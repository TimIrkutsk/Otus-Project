...
* **А2: Обнаружена просроченная партия.**
  * На шаге 6: При сканировании система может показать предупреждение, что срок годности партии истек. Оператор должен отказаться от ее использования и сообщить менеджеру (инициируя процесс утилизации UC2-MVP).
...
**9. Связи в контексте MVP:**

* **Является упрощенной версией** оригинального **UC3: "Пополнить оперативный запас"** в рамках MVP.
* **Сильно зависит от:** **UC1: "Сформировать отчет по сроку годности"**. Без этого отчета оператор не сможет корректно выбрать партию по FEFO.
* **Использует данные из:** **UC4: "Провести инвентаризацию"** (актуальные остатки).
* **Может инициировать:** **UC2-MVP: "Утилизировать партию по отчету"**, если на складе была обнаружена просроченная партия.
...
* **Косвенный:** Менеджер/Бухгалтер (получает отчет).

**3. Предусловия:**

* Оператор авторизован в системе.
* Мобильное устройство оператора имеет соединение с системой (или может работать в автономном режиме).
* Движение на складу и на линии сведено к минимуму (инвентаризация проводится в нерабочие часы или в специально отведенное время).

**4. Основной поток событий:**

| Шаг | Действие Актора | Ответ Системы |
| :-- | :--- | :--- |
| 1 | Use Case начинается, когда Оператор получает **задание "Провести инвентаризацию"** на своем терминале. Задание содержит: тип инвентаризации (сплошная/выборочная), перечень зон (склад, линия) или номенклатуры. | **Триггер:** Система инициирует процесс по расписанию (ежедневно/еженедельно). |
| 2 | Оператор выбирает зону для начала подсчета (например, "Холодильная камера"). | Система отображает схему зоны и список номенклатуры с ожидаемыми остатками. |
...
| 7 | | Система **автоматически корректирует** остатки в системе до фактических. **Ключевой момент:** Корректировка остатков оперативного запаса на линии **не влияет на его виртуальный срок годности**, так как меняется только количество, а не состав партий. |
...
* **Настраиваемость:** Параметр `X` (часы до истечения срока для блокировки) должен быть гибко настраиваемым для разных категорий товаров через интерфейс **UC5**.
* **Безопасность:** Процесс **обратной разблокировки** (снятия статуса) должен быть максимально затруднен и требовать обязательного участия менеджера с указанием веской причины в журнале аудита (журнал регистрации).

**8. Связи:**

* **Включает (include):** **UC13: "Сформировать задание на утилизацию"**. Блокировка партии напрямую ведет к созданию задания для оператора. На диаграмме это отражено стрелкой `include`.
* **Расширяется (extend):** **UC11: "Проверить срок годности"**. Запрос от робота на проверку срока может стать триггером для немедленной блокировки, если проверка выявит нарушение. Это отношение `extend` на диаграмме.
* **Предшествует:** **UC2: "Утилизировать партию"**. Блокировка является обязательным precondition для запуска процесса утилизации системой.
* **Влияет на:** **UC3: "Пополнить оперативный запас"** и **UC8: "Рассчитать автозаказ"**. Заблокированные партии исключаются из расчетов этих процессов.
...
* **Основной:** Система управления запасами.
* **Вторичный:** Менеджер (может получать уведомления о результате расчета).
* **Косвенный:** Логистический центр (получатель итогового заказа).
...
| Шаг | Действие Актора | Ответ Системы |
| :-- | :--- | :--- |
| 1 | Use Case начинается, когда система по расписанию (ежедневно в заданное время) **запускает процедуру расчета автозаказа**. | Система определяет список номенклатурных позиций, для которых настроены правила автозаказа. |
...
| 3 | | Система **применяет алгоритм расчета** для каждой позиции. **Формула:**<br> `Рекомендуемый заказ = Максимальный запас (Max) - Текущий остаток - Незавершенные заказы + Прогноз продаж на срок поставки` |
...
| 8 | | Система **уведомляет менеджера** о готовности черновика заказа для проверки и корректировки (см. UC6). |
| 9 | Use Case завершается. Черновик заказа рассчитан и ожидает действий менеджера. | |
...
* Оператор авторизован в системе.
* Терминал Оператора (мобильное устройство) имеет соединение с системой.

**4. Основной поток событий:**

| Шаг | Действие Актора | Ответ Системы |
| :-- | :--- | :--- |
| 1 | Use Case начинается, когда Оператор получает **задание "Утилизировать партию"** на своем терминале. Задание содержит: наименование полуфабриката, штрих-код партии, локацию (складская ячейка или зона оперативного запаса), причину утилизации (например, "Срок годности истекает через 2 часа"). | Система отображает задание в очереди оператора с высоким приоритетом. |
| 2 | Оператор следует к указанной локации, находит партию по штрих-коду. | |
| 3 | Оператор сканирует штрих-код партии с помощью мобильного устройства. | Система подтверждает, что штрих-код соответствует заданию, и отображает детали партии (срок годности, количество). |
...
* Если утилизирована партия из **оперативного запаса**, система может автоматически инициировать Use Case **UC3: "Пополнить оперативный запас"**.
...
* **Безопасность:** Подтвердить утилизацию может только авторизованный оператор. Для подтверждения может требоваться PIN-код или биометрическая аутентификация (опционально).

**8. Связи:**

* **Включает (include):** UC13. Сформировать задание на утилизацию (система включает этот процесс до начала UC2).
* **Расширяется (extend):** UC7. Заблокировать просроченную партию (процесс блокировки является триггером для начала этого UC).
...
| Шаг | Действие Актора | Ответ Системы |
| :-- | :--- | :--- |
| 1 | Use Case начинается, когда Оператор получает **срочное задание "Пополнить оперативный запас [Наименование]"** на своем терминале. Задание имеет высокий приоритет. Задание содержит: наименование, штрих-код исходной партии, локацию на складе, **количество для перемещения (равное ёмкости контейнера на линии)**. | **Триггер:** Система отследила, что остаток на линии упал ниже точки пополнения. Она автоматически сформировала задание, выбрав партию по **FEFO**. |
| 2 | Оператор немедленно следует к указанной **складской ячейке**, находит партию по штрих-коду. | Система подсвечивает ячейку на схеме склада в приложении оператора. |
| 3 | Оператор сканирует штрих-код партии на складе. | Система подтверждает корректность выбора. **Важно:** Система проверяет, не была ли партия заблокирована после формирования задания (например, другим процессом утилизации). |
| 4 | Оператор забирает **целую упаковку** (или рассчитанное количество) и транспортирует ее к роботу. | |
| 5 | Оператор вскрывает упаковку и **перекладывает полуфабрикаты в контейнер оперативного запаса** на линии. | |
| 6 | Оператор сканирует **штрих-код конкретного контейнера оперативного запаса** на линии, который собирается пополнить. | Система понимает, *куда именно* было добавлено сырье. |
| 7 | Оператор в приложении нажимает **"Подтвердить пополнение"**. Система может запросить подтверждение количества. | |
| 8 | | Система выполняет атомарные действия:<br> 1. **Списывает** перемещенное количество с исходной **складской партии**.<br> 2. **Добавляет** это количество к **остатку оперативного запаса** в указанном контейнере.<br> 3. **Пересчитывает "виртуальный срок годности"** всего содержимого контейнера. **Формула:** Если в контейнере уже был остаток (Y1 с сроком T1), и добавили новую партию (Y2 с сроком T2), новый срок рассчитывается как средневзвешенное: `(Y1*T1 + Y2*T2) / (Y1 + Y2)`. Это критически важно для последующего контроля роботом.<br> 4. Фиксирует акт перемещения. |
| 9 | | Система отправляет **подтверждение роботу**, что запас пополнен и обновляет данные о сроке годности для этого контейнера. |
| 10 | | Система уведомляет оператора: "Контейнер [X] пополнен. Новый срок годности: [дата]". |
| 11 | Use Case завершается. Робот продолжает работу в штатном режиме. | |
...
* **Является частью:** UC9 является **обязательным заключительным этапом** для **UC2: "Утилизировать партию"**. Без подтверждения списания процесс утилизации не считается завершенным.
* **Обобщение:** UC9 — это общий процесс, который может быть вызван из разных контекстов: утилизация просрочки, списание боя, фиксация инвентаризационной недостачи и т.д.

### UC10. Настроить правила автозаказа

**1. Идентификатор и Название:**

**1. Краткое описание:** Этот Use Case описывает процесс определения и конфигурации бизнес-правил, по которым система будет автоматически формировать заявки на пополнение запасов полуфабрикатов с логистического центра (ЛЦ). Технолог или менеджер устанавливает для каждой позиции параметры, минимизирующие риск дефицита и излишков.

**2. Акторы:**

* **Основной:** Технолог / Менеджер по закупкам / Управляющий.
* **Второстепенный:** Система управления запасами.
* **Косвенный:** Логистический центр (получатель сгенерированных заказов).
...
| Шаг | Действие Актора | Ответ Системы |
| :-- | :--- | :--- |
| 1 | Use Case начинается, когда Технолог в меню системы выбирает раздел **"Закупки"** -> **"Правила автозаказа"**. | Система отображает таблицу со списком всех полуфабрикатов и текущими значениями правил для каждого. |
...
| 5 | Технолог настраивает **коэффициенты потребления** (опционально, для сложных алгоритмов): <br> - **Сезонность** (повышающий коэффициент в определенные дни недели или месяцы). <br> - **Прогноз на основе продаж** (вкл/выкл). | |
| 6 | Технолог нажимает кнопку **"Сохранить правило"**. | Система проводит **валидацию** правил: проверяет, что Min < Reorder Point < Max, что минимальная партия не превышает Max запас и т.д. |
...
* **А1: Массовая настройка правил для группы товаров.**
  * Технолог применяет фильтр к списку номенклатуры (например, все товары в категории "Овощи") и нажимает "Настроить для группы". Система позволяет установить общие значения параметров (например, +20% к запасам для всех овощей зимой) для всех отфильтрованных позиций.
* **А2: Настройка правила по шаблону.**
  * Технолог может выбрать готовый шаблон правила (например, "Скоропорт", "Стабильный спрос", "Акционный товар") и применить его к позиции, лишь скорректировав значения.
...
| 8 | | Система **формирует и отправляет ответ** роботу. |
