...
![status_model](assets/Status_model.png)
...
| 7 | Пользователь **принимает решение** на основе отчета: <br> - Сообщает Технологу.<br> - Инициирует процесс утилизации (UC1-MVP).<br> - Переносит партию на более видное место для скорейшего использования (FIFO/FEFO). | |
...
* **А4: Значительное расхождение.**
  * На шаге 6: Если система обнаруживает расхождение по сумме или количеству, превышающее заданный лимит (например, более 10% или более 5000 руб.), она может **заблокировать** эту позицию для дальнейшего разбирательства и уведомить Технолога.
...
| 3 | Технолог заполняет обязательные атрибуты:<br> - **Наименование**<br> - **Единица измерения** - выбирается из выпадающего списка.<br> - **Категория** - для фильтрации и отчетности.<br> - **Срок хранения (часов)** – **критически важный атрибут.** Указывается общий срок для данного полуфабриката в часах. | Система проверяет уникальность наименования. |
...
| 7 | Технолог создает или редактирует зоны, назначая им:<br> - **Уникальный ID/Наименование**<br> - **Тип зоны** – это определяет логику работы системы с зоной.<br> - **Штрих-код зоны** (для сканирования оператором). | Система сохраняет изменения. |
...
* **А3: Массовый импорт данных.**
  * Технолог использует функцию "Импорт из Excel/CSV". Система предоставляет шаблон файла. После загрузки файла система проводит валидацию данных и импортирует их, выводя отчет об успешно добавленных и ошибочных строках.
...
* **Аудит:** Система должна вести **полную историю изменений** по каждому заказу: что именно изменил Технолог, когда и почему (если указана причина). Это критично для анализа эффективности правил автозаказа.
...
* **Уведомления:** Система должна иметь механизм оповещения Технолог о появлении нового черновика заказа (например, за 2 часа до крайнего срока отправки заказов в ЛЦ).
...
* Наступило время запуска фонового задания.
...
| 8 | | Система **уведомляет Технолога** о готовности черновика заказа для проверки и корректировки (см. UC6). |
...
| Шаг | Действие Актора | Ответ Системы |
| :-- | :--- | :--- |
| 1 |  | Система отображает задание в очереди с высоким приоритетом. |
...
| 2 | Оператор сканирует штрих-код партии на складе. | Система подтверждает корректность выбора. **Важно:** Система проверяет, не была ли партия заблокирована после формирования задания (например, другим процессом утилизации). |
...
| 6 | Оператор в приложении нажимает **"Подтвердить пополнение"**. Система может запросить подтверждение количества. | |
...
| 8 | | Система отправляет **подтверждение роботу**, что запас пополнен и обновляет данные о сроке годности для этого контейнера. |
...
* **А1: Робот израсходовал запас до нуля до завершения пополнения.**
  * На любом шаге: Робот может отправить системе аварийное сообщение "Остановка: нет ингредиента X". Система повышает приоритет задания до "КРИТИЧЕСКИЙ", уведомляет Технолога. Оператор должен завершить пополнение в максимально сжатые сроки.
...
| Шаг | Действие Актора | Ответ Системы |
| :-- | :--- | :--- |
| 1 | Оператор в мобильном приложении выбирает функцию **"Зафиксировать списание"**. | Система предлагает выбрать **причину списания** из справочника: `Просрочка`, `Брак`, `Бой/Порча`, `Прочее`. |
| 2 | Оператор выбирает причину списания. | |
| 3 | Оператор сканирует **штрих-код партии**, подлежащей списанию. | Система распознает партию, отображает ее данные: наименование, текущий остаток, срок годности. **Важно:** Если причина "Просрочка", система проверяет, что статус партии = "Заблокирована". |
...
| 6 | Оператор прикрепляет **фотографию** испорченного товара (опционально, но рекомендуется для аудита). | |
| 7 | Оператор нажимает кнопку **"Подтвердить списание"**. | Система запрашивает финальное подтверждение действия, указывая сумму списания. |
| 8 | Оператор подтверждает действие. | |
| 9 | | Система выполняет атомарные действия:<br> 1. **Списывает** указанное количество с остатка партии.<br> 2. **Меняет статус** партии (если списан весь остаток, статус меняется на "Утилизирована").<br> 3. **Фиксирует акт списания** в базе данных с обязательными реквизитами: дата, время, партия, причина, количество, ответственный, стоимость, комментарий, фото.<br> 4. **Обновляет** отчетные данные и остатки в реальном времени. |
...
* **А3: Попытка списать не заблокированную партию с нормальным сроком.**
  * На шаге 4: Если причина "Просрочка", но срок годности не истек и статус не "Заблокирована", система **отклоняет запрос** и требует дополнительного подтверждения от Технолога (ввод кода или push-уведомление Технологу). Это защита от ошибок и злоупотреблений.
...
* **Аудит и безопасность:** Все акты списания должны храниться в неизменяемом виде. Должна быть реализована **ролевая модель**, запрещающая обычным операторам списывать товары без ограничений (только по заблокированным системой партиям или по утвержденным Технологом актам).
...
* **Конфиденциальность:** Акт списания с фотографией должен быть доступен только авторизованным пользователям (Технологам, бухгалтерам).
...
* **А2: Настройка правила по шаблону.**
  * Технолог может выбрать готовый шаблон правила и применить его к позиции, лишь скорректировав значения.
...
* **А3: Срок годности истекает в течение периода готовки.**
  * На шаге 7: Система может быть настроена на проверку "с запасом". Если срок годности истекает раньше, чем истечет срок реализации готового блюда (например, через 2 часа), система может вернуть ответ-предупреждение. Робот может либо продолжить работу, либо также остановиться для решения оператора.
...
* **А1: Ошибка интеграции (API недоступно).**
  * На шаге 4: Если система не получает ответа от API ЛЦ (таймаут, ошибка сети), она выполняет **повторные попытки отправки** по заданному графику (например, через 1 мин, 5 мин, 15 мин). После исчерпания попыток заказ переводится в статус "Ошибка отправки", и Технолог получает уведомление.
...
* **А3: Частичное подтверждение от ЛЦ.**
  * ЛЦ может подтвердить заказ, но изменить количество или дату поставки. Система должна зафиксировать этот факт в истории заказа и уведомить Технолога.
...
* **Устойчивость к сбоям:** Механизм отправки должен использовать **очередь сообщений (message queue)**. Это гарантирует, что заказы не будут потеряны при кратковременной недоступности системы или ЛЦ.
...
*   **А3: Массовое попадание под критерии.**
    *   Если в результате сканирования выявлено аномально большое количество партий под утилизацию, система, помимо создания заданий, может отправить **email- или SMS-уведомление Технологу** о критической ситуации.
...
*   **Конфигурируемость:** Параметр `X` (часов до истечения срока) должен быть легко настраиваемым через административный интерфейс системы для разных категорий товаров.
